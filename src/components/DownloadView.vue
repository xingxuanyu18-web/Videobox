<template>
  <div class="flex-1 flex overflow-hidden">
    ...
    <section class="w-[65%] flex flex-col bg-surface relative">
        ...
        <div class="max-w-xl mx-auto w-full flex flex-col gap-6">
          ...
          <div class="flex flex-col gap-1.5 mb-1">
            <h1 class="text-2xl font-extrabold tracking-tight text-text-primary">
              视频<span class="gradient-text">下载</span>
            </h1>
            <p class="text-text-secondary text-sm">粘贴链接，即刻解析全球 1000+ 平台视频</p>
          </div>
          ...
          <div class="flex flex-col gap-3">
            <div class="relative glow-ring rounded-2xl">
              <textarea
                ...
                class="w-full h-28 glass-card p-4 text-text-primary placeholder:text-text-muted resize-none text-sm transition-all duration-300 focus:shadow-glow-primary"
            ...
            <div class="flex gap-3">
              <button
                class="flex-1 flex items-center justify-center gap-2 rounded-xl h-11 bg-bg-highlight text-text-secondary hover:text-text-primary hover:bg-bg-card transition-all duration-200 font-semibold text-sm border border-border-subtle hover:border-border-default"
                @click="pasteUrl"
              >
              ...
              <button
                class="flex-[2] flex items-center justify-center gap-2 rounded-xl h-11 font-bold text-sm transition-all duration-300 gradient-primary text-white shadow-glow-primary hover:shadow-glow-primary hover:brightness-110 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed disabled:shadow-none"
                :disabled="isParsing"
                @click="parseVideo"
              >
                <span v-if="isParsing" class="material-symbols-outlined text-[18px] animate-spin">sync</span>
          ...
          <div class="flex flex-col gap-2.5 mt-1">
            <span class="text-2xs font-semibold text-text-muted uppercase tracking-[0.15em] ml-1">支持平台</span>
            <div class="flex items-center gap-1.5 flex-wrap">
              <span v-for="platform in platforms" :key="platform.name"
                class="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-bg-card border border-border-subtle hover:border-border-default hover:bg-bg-highlight transition-all duration-200 cursor-default group">
                <span class="material-symbols-outlined text-[16px] text-primary group-hover:text-primary-hover transition-colors">{{ platform.icon }}</span>
                <span class="text-[11px] font-medium text-text-secondary group-hover:text-text-primary transition-colors">{{ platform.name }}</span>
              </span>
            </div>
          </div>
          ...
          <div v-if="videoInfo" class="mt-2 flex flex-col gap-4">
            ...
            <!-- Video Info Card -->
            <div class="flex gap-4 p-4 glass-card animate-fade-in">
              <div
                class="w-44 h-28 rounded-xl bg-bg-highlight bg-cover bg-center flex-shrink-0 relative overflow-hidden shadow-elevated"
                :style="{ backgroundImage: `url(${videoInfo.thumbnail})` }"
              >
                <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
                <div v-if="videoInfo.duration" class="absolute bottom-2 right-2 bg-black/70 backdrop-blur-md text-white text-2xs font-mono font-semibold px-2 py-0.5 rounded-lg">
                  {{ formatDuration(videoInfo.duration) }}
                </div>
              </div>
              <div class="flex flex-col flex-1 justify-between py-1">
                <div>
                  <h4 class="font-bold text-sm text-text-primary leading-snug line-clamp-2">{{ videoInfo.title }}</h4>
                  <p v-if="videoInfo.uploader" class="text-xs text-text-secondary mt-1.5 flex items-center gap-1">
                    <span class="material-symbols-outlined text-[14px]">account_circle</span>
                    <span>{{ videoInfo.uploader }}</span>
                  </p>
                </div>
                <div class="flex items-center gap-4 text-2xs text-text-muted font-mono">
            ...
            <div class="flex flex-col gap-2.5">
              <span class="text-2xs font-semibold text-text-muted uppercase tracking-[0.15em]">视频格式 · {{ videoInfo.formats?.length || 0 }} 个</span>
              <div class="grid grid-cols-2 gap-1.5">
                <label
                  v-for="format in videoInfo.formats"
                  :key="format.formatId"
                  class="flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all duration-200 border"
                  :class="selectedFormat?.formatId === format.formatId
                    ? 'bg-primary-subtle border-primary/40 shadow-glow-primary'
                    : 'bg-bg-card border-border-subtle hover:border-border-default hover:bg-bg-highlight'"
                  @click="selectVideoFormat(format)"
                >
                  <div class="flex items-center gap-2.5">
                    <div class="size-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all"
                      :class="selectedFormat?.formatId === format.formatId ? 'border-primary' : 'border-border-strong'">
                      <div v-if="selectedFormat?.formatId === format.formatId" class="size-2 rounded-full bg-primary animate-fade-in"></div>
                    </div>
                    <span class="text-xs font-semibold" :class="selectedFormat?.formatId === format.formatId ? 'text-primary' : 'text-text-secondary'">{{ format.quality }}</span>
                  </div>
                  <span class="text-2xs font-mono" :class="selectedFormat?.formatId === format.formatId ? 'text-primary' : 'text-text-muted'">{{ format.filesize ? formatFileSize(format.filesize) : '-' }}</span>
                </label>
              </div>
            </div>
            ...
            <!-- Download Button -->
            <button
              class="w-full flex items-center justify-center gap-2.5 rounded-2xl h-14 font-bold text-base transition-all duration-300 mt-2 gradient-primary text-white shadow-glow-primary hover:shadow-glow-primary hover:brightness-110 active:scale-[0.98] disabled:opacity-30 disabled:cursor-not-allowed disabled:shadow-none disabled:active:scale-100"
              :disabled="(!selectedFormat && !selectedAudioFormat && selectedSubtitles.length === 0) || isDownloading"
              @click="startDownload"
            >
              <span v-if="isDownloading" class="material-symbols-outlined text-[24px] animate-spin">sync</span>
              <span v-else class="material-symbols-outlined text-[24px]">download</span>
              <span>{{ isDownloading ? '正在下载...' : (downloadMode === 'audio' ? '下载纯音频' : (downloadMode === 'subtitle' ? '下载字幕' : '开始下载')) }}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      >
        <div class="max-w-xl mx-auto flex flex-col gap-2">
          ...
          <div class="h-1.5 w-full bg-bg-highlight rounded-full overflow-hidden">
            <div
              class="h-full rounded-full transition-all duration-500 gradient-primary shadow-glow-primary"
    ...
    <section class="w-[40%] bg-bg-base flex flex-col pt-6 px-5 pb-6 border-l border-border-subtle">
      <div class="flex items-center justify-between mb-5">
        <h2 class="font-bold text-sm text-text-primary">任务队列</h2>
        <div class="flex items-center gap-2">
          <span class="text-2xs font-mono font-semibold text-primary bg-primary-subtle px-2 py-1 rounded-lg">
            {{ activeTaskCount }} 进行中
          </span>
          <button
            class="p-1.5 text-text-muted hover:text-text-primary rounded-lg hover:bg-bg-highlight transition-all duration-200"
      ...
      <!-- Storage Info -->
      <div class="mt-auto pt-4 border-t border-border-subtle flex items-center justify-between text-2xs text-text-muted">
        <div class="flex items-center gap-1.5">
          <span class="material-symbols-outlined text-[14px]">storage</span>
          <span>{{ storageInfo }}</span>
        </div>
        <div class="flex items-center gap-1.5">
          <span class="font-mono text-text-muted truncate max-w-[160px]">{{ downloadDir }}</span>
          <button
            class="p-1 hover:bg-bg-highlight rounded-lg transition-all duration-200 hover:text-text-primary"
...
async function openFileLocation(task: DownloadTask) {
  if (task.filePath) {
    const folder = task.filePath.substring(0, task.filePath.lastIndexOf('\\'))
    await window.electronAPI.shell.openPath(folder)
  }
}
